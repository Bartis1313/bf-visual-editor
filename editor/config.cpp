...
void VisualEnvironmentEditor::SerializeEmitterSnapshot(json& j, const EmitterEditData::Snapshot& s)
{
...
void VisualEnvironmentEditor::DeserializeEmitterSnapshot(const json& j, EmitterEditData::Snapshot& s)
{
...
void VisualEnvironmentEditor::SerializeColorSnapshot(json& j, const EmitterEditData::ColorSnapshot& s)
{
...
void VisualEnvironmentEditor::DeserializeColorSnapshot(const json& j, EmitterEditData::ColorSnapshot& s)
{
...
#undef SAVE_GLOBAL_COMPONENT
        ...
        json emittersArray = json::array();

        for (const auto& [dataPtr, edit] : m_EmitterMap)
        {
            if (!edit.modified)
                continue;

            json emitterJson;
            emitterJson["name"] = edit.name;

            EmitterEditData::Snapshot currentTemplate;
            EmitterEditData::ColorSnapshot currentColor;
            CaptureCurrentEmitterState(edit, currentTemplate, currentColor);

            json templateJson;
            SerializeEmitterSnapshot(templateJson, currentTemplate);
            emitterJson["template"] = templateJson;

            if (currentColor.exists)
            {
                json colorJson;
                SerializeColorSnapshot(colorJson, currentColor);
                emitterJson["colorProcessor"] = colorJson;
            }

            emittersArray.push_back(emitterJson);
        }

        if (!emittersArray.empty())
        {
            root["emitters"] = emittersArray;
            LOG_INFO("SaveConfig: Saved %zu modified emitters", emittersArray.size());
        }
...
#undef LOAD_GLOBAL_COMPONENT
        ...
        {
            LOG_INFO("LoadConfig: Found %zu emitters in config", root["emitters"].size());

            m_PendingEmitterEdits.clear();
            ...
            {
                std::string name = emitterJson.value("name", "");
                if (name.empty())
                    continue;
                ...
                for (auto& [dataPtr, edit] : m_EmitterMap)
                {
                    if (edit.name == name)
                    {
                        if (emitterJson.contains("template"))
                        {
                            EmitterEditData::Snapshot loadedTemplate;
                            DeserializeEmitterSnapshot(emitterJson["template"], loadedTemplate);
                            loadedTemplate.RestoreTo(edit.data);
                        }

                        if (emitterJson.contains("colorProcessor") && edit.colorProcessor)
                        {
                            EmitterEditData::ColorSnapshot loadedColor;
                            DeserializeColorSnapshot(emitterJson["colorProcessor"], loadedColor);
                            loadedColor.RestoreTo(edit.colorProcessor);
                        }

                        edit.modified = true;
                        LOG_INFO("LoadConfig: Applied emitter '%s'", name.c_str());
                        appliedCount++;
                        found = true;
                        break;
                    }
                ...
                if (!found)
                {
                    PendingEmitterEdit pending;
                    pending.name = name;
                    pending.hasColorData = false;

                    if (emitterJson.contains("template"))
                        DeserializeEmitterSnapshot(emitterJson["template"], pending.templateData);

                    if (emitterJson.contains("colorProcessor"))
                    {
                        DeserializeColorSnapshot(emitterJson["colorProcessor"], pending.colorData);
                        pending.hasColorData = true;
                    }

                    m_PendingEmitterEdits.push_back(std::move(pending));
                    pendingCount++;
                }
            }

            LOG_INFO("LoadConfig: Applied %d emitters, %d pending", appliedCount, pendingCount);
        }
